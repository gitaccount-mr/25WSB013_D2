...
void loop() {
  ...
  // Always listen for incoming radio commands (non-blocking)
  if (radio.available()) {
    radio.read(&receivedCommand, sizeof(receivedCommand));
  }

  // Everything else only runs at the 200ms interval
  if (now - lastTransmitTime >= TRANSMIT_INTERVAL) {
    ...
    // Relay control based on last received command
    ...
    // Determine display value for radio response
    int dispRadio;
    if (receivedCommand == '1') {
